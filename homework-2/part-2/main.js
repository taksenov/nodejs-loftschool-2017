...
// main code thread ==============================================================================
const asyncWorker = async (inDir, outDir, isDelete) => {
    try {
        handleCombineMusicCollection(inDir, outDir);
        await Promise.all(copyPromices);
        if (isDelete) {
            await handleDeleteFilesInInputDir(inDir, isDelete);
            await handleDeleteInputDir(inDir);
        }
    } catch (error) {
...
asyncWorker(inDir, outDir, isDelete);
